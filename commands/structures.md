...
2. **Send `get_analysis_coverage` and `get_content_structures` in ONE message — they do not
   depend on each other.** Both come back in a single round-trip instead of two.
3. **Read coverage before you write the answer** — structures only exist for **analysed**
   reels. Thin coverage means say so and mark the answer low-confidence.
   `get_content_structures` holds the shapes themselves.
4. For any structure worth copying, pull a real example so the user sees the actual beats:
   `query_posts_by_tag` to find the reels, then `get_post_transcript` for one of them. These
   two DO depend on each other — you cannot ask for a transcript until you know which reel —
   so they stay two turns. If you want transcripts for several reels, ask for all of them in
   one message.
5. Apply the skill's PLAYBOOK rigor rules (Step 3, Step 7) — load ONLY those two files,
   ...
   respect the anti-mimicry and stage rules (Step 4, Step 5 —
   `playbook/step-04-stage.md` and `playbook/step-05-differentiate.md`) — a structure that works for a
